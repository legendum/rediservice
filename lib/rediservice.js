...
function Rediservice( servicesDSL ) {
  ...
  this.run = (selector, opts) => {
    ...
    for ( var serviceName in services ) {
      ...
      if ( opts.debug ) { console.log( JSON.stringify({running: Object.keys(this._running)}, null, 2) ); }
    }
  };
  ...
  this.service = (serviceName, serviceDSL) => {
    ...
    let start = (opts) => { // e.g. opts could be {debug: true} for verbose logs
      ...
      if ( opts.debug ) {
        this.on( serviceName, (data) => console.log( JSON.stringify({service: serviceName, data: data}, null, 2) ) );
      }
  ...
  // setup Redis then call the services DSL function to define our microservices
  this.setup = (redisOpts) => {
    ...
    if ( types.isFunction(servicesDSL) ) {
      servicesDSL.call( this, redisOpts ); // (pass the redisOpts just for info)
    }

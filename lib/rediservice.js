...
function Rediservice( redisOpts ) {
  ...
  function isMatch( data, match ) {
    for ( var k in match ) {
      let v = match[k];
      if ( v === true ) {
        if ( types.isNil( data[k] ) ) { return false; } // missing data field
      } else if ( v === false ) {
        if ( !types.isNil( data[k] ) ) { return false; } // existing data field
      } else {
        if ( data[k] !== v ) { return false; } // data field value doesn't match
      }
    }
    return true;
  }

...
function Rediservice( redisURL, redisOpts ) {
  ...
  this._exports = {
    ...
    services: (selector) => {
      let found = {};
      for (var serviceName in this._exports) {
        if ( serviceName === 'run' || serviceName === 'services' ) { continue; }
        if ( selector && !serviceName.match(selector) ) { continue; }
        found[serviceName] = this._exports[serviceName];
      }
      return found;
    }
  ...
  this.service = (serviceName, callDSL) => {
    ...
    if ( serviceName === 'run' || serviceName === 'services' ) {
      throw new Error('service cannot be called "run" or "services" - sorry!');
    }
    ...
    this._exports[serviceName] = start;
    return start;
  };
